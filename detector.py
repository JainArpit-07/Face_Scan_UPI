...
def recognize_faces(image_location, model: str = "hog", encodings_location: Path = DEFAULT_ENCODINGS_PATH) -> None:
    if isinstance(image_location, str):
        input_image = face_recognition.load_image_file(image_location)
    else:
        # Convert the frame from BGR to RGB
        input_image = cv2.cvtColor(image_location, cv2.COLOR_BGR2RGB)
    ...
    for bounding_box, unknown_encoding in zip(input_face_locations, input_face_encodings):
        name = _recognize_face(unknown_encoding, encodings_location)
        if not name:
            name = "Unknown"
...
def _display_face(draw, bounding_box, name):
    ...
    draw.rectangle(((text_left, text_top), (text_right, text_bottom)), fill="orange", outline="blue")
    draw.text((text_left, text_top), name, fill=TEXT_COLOR)

def _recognize_face(unknown_encoding, encodings_location):
...
def capture_video(model: str = "hog", encodings_location: Path = DEFAULT_ENCODINGS_PATH):
    ...
    while True:
        ...
        recognize_faces(frame, model, encodings_location)
